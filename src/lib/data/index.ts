...
export async function getProductByHandle(
  ...
  const queryParams =
    "categories,variants,variants.prices,variants.options,variants.images,options,options.values,collection,images,profiles,tags"
 ...
 * @param categoryId (Array) - The array of IDs of the categories
 * @returns response (object) - An object containing the products and the number of products in the collection
 * @returns nextPage (number) - The offset of the next page of products
 */
export async function getProductsByCollectionHandle({
    ...
    categoryIds = categories?.map(
      (category) =>
        product_categories.find(
          (p_category) =>
            p_category.name.toLowerCase() === category.toLowerCase()
        )?.id ?? ""
    )
  }
  ...
  const { response, nextPage } = await getProductsList({
    ...
    queryParams: {
      ...
      limit,
      expand:
        "collection,images,options,profiles,tags,type,variants,variants.options,variants.prices,categories",
...
/**
 * Fetches the count of products in each brand in a collection.
 * @param pageParam (number) - The offset of the products to retrieve
 * @param handle (string) - The handle of the collection to retrieve
 * @param cartId (string) - The ID of the cart
 * @returns hugoCount (number) - The count of products in HUGO
 * @returns bossCount (number) - The count of products in BOSS
 * @returns totalCount (number) - The total count of products
 */
export async function getProductCountByBrandByCollectionHandle({
  pageParam = 0,
  limit = 1,
  handle,
  cartId,
  ...
  limit?: number
  cartId?: string
}): Promise<{ hugoCount: number; bossCount: number; totalCount: number }> {
  let categoryIds: string[] = []

  const [{ id }, { product_categories }] = await Promise.all([
    getCollectionByHandle(handle).then((res) => res.collections[0]),
    getCategoriesList(0, 1000),
  ])

  categoryIds.push(
    product_categories.find(
      (category) => category.name.toLowerCase() === "hugo"
    )?.id!
  )

  const [hugoCount, totalCount] = await Promise.all([
    getProductsList({
      pageParam,
      queryParams: {
        collection_id: [id],
  ...
  return {
    hugoCount,
    bossCount: totalCount - hugoCount,
    totalCount,
  }
}
